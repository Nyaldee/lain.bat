...
"%Temp%\NVIDIA Profile Inspector.exe" "%Temp%\NvidiaBaseProfile.nip"
del "%Temp%\NVIDIA Profile Inspector.exe"
del "%Temp%\NvidiaBaseProfile.nip"
goto :NVIDIA
)
if errorlevel 4 (
:: https://forums.guru3d.com/threads/windows-line-based-vs-message-signaled-based-interrupts-msi-tool.378044/
curl -s -L -o "%Temp%\MSI utility v3.exe" "https://github.com/Nyaldee/lain.bat/raw/main/call/MSI_util_v3.exe"
start /wait "" "%Temp%\MSI utility v3.exe"
del "%Temp%\MSI utility v3.exe"
goto :NVIDIA
)
if errorlevel 3 (
...
start /wait "" "%Temp%\NVCleanstall.exe"
del "%Temp%\NVCleanstall.exe"
goto :NVIDIA
)
if errorlevel 2 (
...
start /wait "" "%Temp%\wushowhide.diagcab"
del "%Temp%\wushowhide.diagcab"
rd "%Temp%\msdtadmin" /s /q
goto :NVIDIA
)
...
echo.	 Disable everything except Interrupt Moderation, can limit the connection to 100mbps.
echo.
pause
goto :WSETTINGS
)
...
echo.+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
echo. → Les erreurs sont normales.
...
pause
del "%Temp%\PowerRun.exe"
del "%Temp%\RestoreServices.reg"
goto :SERVICES
)
if errorlevel 10 (
...
call "%Temp%\Disable services.bat"
del "%Temp%\Disable services.bat"
pause
goto :SERVICES
)
...
:REG
echo.===============================================================================
echo.Configuration de Windows via le registre ?
choice /C:YNV /N /M "Setting up Windows via the registry ? ['Y'es/'N'o/'V'iew modifications] : "
if errorlevel 3 (
cls
echo.===============================================================================
...
call "%Temp%\PowerPlan.bat"
del "%Temp%\PowerPlan.bat"
powercfg -h off
powercfg /list
)
...
if errorlevel 2 goto :END
dism /online /Disable-Feature /FeatureName:"Windows-Defender-ApplicationGuard" /Quiet /NoRestart
dism /online /Disable-Feature /FeatureName:"VirtualMachinePlatform" /Quiet /NoRestart
dism /online /Disable-Feature /FeatureName:"HypervisorPlatform" /Quiet /NoRestart
:: Others
...
dism /online /Remove-Capability /CapabilityName:Microsoft.Windows.MSPaint~~~~0.0.1.0 /Quiet /NoRestart
dism /online /Remove-Capability /CapabilityName:Microsoft.Windows.SnippingTool~~~~0.0.1.0 /Quiet /NoRestart
chcp 437>nul
...
reg delete "HKLM\SYSTEM\CurrentControlSet\Services\STR" /f
echo The operation completed successfully.
echo Redémarrez pour que les modifications prennent effets.
...
:END
echo.===============================================================================
echo.Suppression de tous les fichiers, dossiers inutiles et prévenir certaines créations ?
echo.Delete all unnecessary files, folders and prevent certain creations ?
choice /C:YN /N /M "Remove ? ['Y'es/'N'o] : "
if errorlevel 2 goto :END
curl -s -L -o "%Temp%\Cleanup.bat" "https://github.com/Nyaldee/lain.bat/raw/main/call/Cleanup.bat"
call "%Temp%\Cleanup.bat"
del "%Temp%\Cleanup.bat"
...
reg import "%Temp%\sageset.reg"
del "%Temp%\sageset.reg"
...
reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer\DisallowRun" /v "Edge" /t REG_SZ /d "msedge.exe" /f
...
net stop "UsoSvc" & sc config "UsoSvc" start= Demand
...
icacls "%WinDir%\System32\wuaueng.dll" /grant:r %username%:F
ren "%WinDir%\System32\wuaueng.dll" "wuaueng.bak"
takeown /F "%WinDir%\System32\WaasMedicSvc.dll"
icacls "%WinDir%\System32\WaasMedicSvc.dll" /grant:r %username%:F
...
reg delete "HKU\.DEFAULT\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Accent" /f
...
reg delete "HKU\.DEFAULT\SOFTWARE\Microsoft\Windows\DWM" /f
reg delete "HKLM\SOFTWARE\Microsoft\Windows\Dwm" /f
reg delete "HKU\.DEFAULT\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes\Personalize" /f
reg delete "HKCU\Control Panel\Colors" /f
reg delete "HKU\.DEFAULT\Control Panel\Colors" /f
reg delete "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v "Background" /f
...
reg delete "HKLM\Software\Policies\Microsoft\Windows\Personalization" /v "PersonalColors_Background" /f
...
reg delete "HKLM\SOFTWARE\Policies\Microsoft\Windows\DWM" /f
...
reg import "%Temp%\BlackViolet.reg"
del "%Temp%\BlackViolet.reg"
goto :END
)
curl -s -L -o "%Temp%\Pitch Black Theme.reg" "https://github.com/Nyaldee/lain.bat/raw/main/call/PitchBlackTheme.reg"
reg import "%Temp%\Pitch Black Theme.reg"
del "%Temp%\Pitch Black Theme.reg"
...
data.msn.com
    ...
    findstr /v /i /c:"127.0.0.1 %%a" "%hostsPath%" > "%hostsPath%.tmp" && move /y "%hostsPath%.tmp" "%hostsPath%" >nul
)
curl -s -L -o "%Temp%\FirewallRulesRemove.bat" "https://github.com/Nyaldee/lain.bat/raw/main/call/FirewallRulesRemove.bat"
call "%Temp%\FirewallRulesRemove.bat"
del "%Temp%\FirewallRulesRemove.bat"
pause
)
...
data.msn.com
    ...
    findstr /i /c:"127.0.0.1 %%a" "%hostsPath%" >nul || echo 127.0.0.1 %%a >> "%hostsPath%"
)
curl -s -L -o "%Temp%\FirewallRulesAdd.bat" "https://github.com/Nyaldee/lain.bat/raw/main/call/FirewallRulesAdd.bat"
call "%Temp%\FirewallRulesAdd.bat"
del "%Temp%\FirewallRulesAdd.bat"
pause
